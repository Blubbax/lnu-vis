...
function drawSunburst(data) {
    ...
    function clicked(event, p) {
        ...
        path.transition(t)
            ...
            .attr("fill-opacity", d => arcVisible(d.target) ? (d.children ? 1 : 0.8) : 1)
    ...
    function labelTransform(d, dChildCheck) {
        ...
        if (dChildCheck.children) {
          return `rotate(${x - 90}) translate(${y},0) rotate(${x <= 180 ? 270 : 90})`;
        } else {
    ...
    //         .padAngle(d => Math.min((d.x1 - d.x0) / 2, 0.005))
    //         .padRadius((800 / 6) * 1.5)
    //         .innerRadius(d => d.y0 * (800 / 6))
    //         .outerRadius(d => Math.max(d.y0 * (800 / 6), d.y1 * (800 / 6) - 1))
    // }
